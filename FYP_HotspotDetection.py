...
for i in range(len(img_list)):
    ...
    # box_x = img_width*float(coord[1])
    # box_width = img_width*float(coord[3])
    # box_y = img_height*float(coord[2])
    # box_height = img_height*float(coord[4])
    ...
    for i in range(num_segments):
        for j in range(num_segments):
            ...
            # if segment_means[k] < lower_boundary:
            #     # Draw the boundary box on the original image
            #     color = (0, 255, 0) # Red color
            #     thickness = 2
            #     cv2.rectangle(module, (x_start, y_start), (x_end, y_end), color, thickness)
    ...
    plt.savefig(r"C:\Users\jingc\OneDrive\Desktop\FYP_HS\histogram" + '\\' + img_name_w_filetype)
    plt.clf()
